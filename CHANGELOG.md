...
- `reep/scripts/export-csv.py`: `_resolve_reep_id` now short-circuits when the `qid` slot already holds a `reep_*` identifier — custom entities from `fetch-custom-entities.py` store their reep_id in the qid slot as a fallback since they have no Wikidata QID.
