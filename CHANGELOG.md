...
### Data
- **14,944 previously-orphaned custom entities** now appear in the public CSVs for the first time. These are entities created directly in D1 by match scripts since v2.3.0 (2,023 `transfermarkt-datasets` players, 12,899 `opta` players, 8 `sdapi-mapping` competitions) which never flowed back into `data/json/*.json` and were therefore invisible to `export-csv.py`. A new `fetch-custom-entities.py` step now pulls them across. people.csv grew from 429,785 → 444,707 rows; competitions.csv from 191 → 213.
...
- **149 `position_detail` backfills** + **8,853 new player aliases** harvested from dcaribou and salimt name variants (stored in `custom_aliases` under the `transfermarkt-datasets` provider).
- **15 new competition entities** created from dcaribou for domestic cups and UEFA qualifying rounds that were missing from reep entirely: Belgian Super Cup, Danish Cup, Greek Football Cup, KNVB Cup, Johan Cruijff Shield, Taça da Liga, Supertaça Cândido de Oliveira, Russian Cup, Russian Super Cup, Scottish Cup, Ukrainian Cup, Ukrainian Super Cup, UEFA Champions/Europa/Conference League qualifying rounds. Source: `transfermarkt-datasets`, all with `key_transfermarkt` custom_ids.
- **342 `date_of_birth` placeholder fills** — reep entities with year-precision `YYYY-01-01` stubs updated from TM's specific dates where the year matches. Further 176 DOB gap candidates (NULL reep DOBs and 24 year-mismatch cases) exported to `reep-custom/data/tm-dob-review.csv` for manual review.

### Data quality fixes
- **Three famous clubs split from Wikidata-fallback stubs**. Reep had been serving `FC Barcelona` / `CA Boca Juniors` / `Club Atlético Estudiantes` entities that were actually `Q5424838` (FC Barcelona Beach Soccer), `Q5008937` (Boca Juniors women's team), and `Q118951641` (Estudiantes from Resistencia, Chaco) — the SPARQL label service was returning the bare "FC Barcelona" / "CA Boca Juniors" / "Club Atlético Estudiantes" strings for these smaller/auxiliary entities, masking the fact that the real men's clubs (`Q7156` Futbol Club Barcelona, `Q170703` Boca Juniors, `Q8206935` Estudiantes de Río Cuarto) were missing from reep entirely. All three real clubs now exist as new entities with `source='manual-wikidata'`, proper founded dates (1899/1905/1912), stadium names (Camp Nou, La Bombonera), country, aliases, and their Transfermarkt IDs (131/189/14602). The three stub entities were renamed to `FC Barcelona (beach soccer)` / `CA Boca Juniors (women)` / `Club Atlético Estudiantes (Resistencia)` for disambiguation. Known limitation: upstream Wikidata still has miscategorised provider claims (sofifa/sportmonks/thesportsdb describing the main men's club but attached to the beach-soccer/women's QIDs) that will reappear on the stubs after the next weekly refresh — proper fix is a Wikidata edit, out of scope here.
- **AFCN reep entity renamed** from `Afrique corrompu fédération` (stale Wikidata vandalism) to `Africa Cup of Nations`. The entity's Wikidata QID Q83145 has since been corrected upstream but reep's `name_en` was still holding the old label.
- **Duplicate SEN1 TM mapping removed** from the seasonal entity `reep_l9eef4be6` (2025–26 Senegal Ligue 1), which had inherited its parent competition's Transfermarkt code via Wikidata's P2446 inheritance. The canonical mapping stays on `reep_l00604bd8` (Senegal Premier League).
- **2 corrupt TM external_ids deleted**: `reep_p3c6d037f` Pablo Lúcio had `external_id = 'Pablo Lúcio'` (literal name in the ID field) and `reep_tbdb84997` One Taguig F.C. had a full Transfermarkt URL. The Taguig case was repaired to the correct numeric ID `116298` (parsed from the URL path). Both came from Wikidata P2446 data-entry errors.
...
- `reep/scripts/fetch-custom-entities.py`: new script that pulls `source != 'wikidata'` entities from D1 and merges them into `data/json/*.json` so `export-csv.py` picks them up. Idempotent — runs after `fetch-custom-ids.py` in the standard pipeline. Fills a long-standing gap where 15K+ match-script-created entities lived in D1 but never reached the public CSVs.
- `reep/scripts/export-csv.py`: `_resolve_reep_id` now short-circuits when the `qid` slot already holds a `reep_*` identifier — custom entities from `fetch-custom-entities.py` store their reep_id in the qid slot as a fallback since they have no Wikidata QID. Same pattern applied to the Source 1 alias export loop so `aliases_en` values on custom entities propagate to `names.csv` correctly.
- `reep-custom/scripts/validate-db.py`: added a new `format` test for `transfermarkt` IDs that enforces pure-numeric values for player and team entities (competitions use alphanumeric codes like `GB1`, so they're excluded). Caught the two corruptions above in its first run.
- **Review CSVs exported for human triage**: `reep-custom/data/tm-dob-review.csv` (176 unfillable DOB gaps), `reep-custom/data/tm-merge-candidates.csv` (47 cases where a single TM ID maps to 2+ reep entities — 12 player + 32 team groups, some legitimate merge candidates like `Kevin Pytlik` vs its Japanese transliteration, others cross-matched TM page errors), and `reep-custom/data/tm-dob-mismatch-review.csv` (1,024 cases where reep and TM have different specific dates — notably 82 day/month swap patterns and 119 year-off-by-1 cases, suggesting Wikidata data-entry errors).
